...
spec = describe "Match a line" $ do
  ...
  it "Should deal with quotes on end as well" $
     getDuration "[silencedetect @ 0x1900280] silence_end: 1249.91 | silence_duration: 15.7841'"
      `shouldBe` 15.7841

  it "Shouldn't filter this line" $
      takeOnlyLines "[silencedetect @ 0xcf3280] silence_end: 415.498 | silence_duration: 69.6312'"
      `shouldBe` True
